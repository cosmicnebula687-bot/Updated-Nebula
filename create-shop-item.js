...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(opt =>
      opt
        .setName('id')
        .setDescription('Unique item ID (no spaces)')
        .setRequired(true)
    )
    .addStringOption(opt =>
      opt
    ...
    .addBooleanOption(opt =>
      opt
        .setName('usable')
        .setDescription('Is this item usable?')
        .setRequired(false)
    )
  ...
  async execute(interaction) {
      ...
      return interaction.reply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              `An item with ID **${itemId}** already exists.`
            )
    ...
    const sellPrice =
      interaction.options.getInteger('sell-price') ?? 0;
    ...
    const item = await ShopItem.create({
      ...
      rarity:
        interaction.options.getString('rarity') || 'Common',
      emoji:
        interaction.options.getString('emoji') || '📦',
      sellable: sellPrice > 0,
      usable:
        interaction.options.getBoolean('usable') || false,
    });
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: '📦 Item',
          value: `${item.emoji} **${item.name}** ${rarityEmoji(
            item.rarity
          )}`,
          inline: true,
        },
        {
          name: '💰 Price',
          value: `☄️ ${item.price.toLocaleString()}`,
          inline: true,
        },
        {
          name: '🆔 ID',
